...
### Projects
- **Sonar** - A federated learning server running on the blockchain that handles all campaign requests, holding Bounty in trust.  This library communicates with the Capsule to generate PGP keys and deliver the final, trained results back to the Buyer. It also communicates with miners, collecting Gradients and distributing Bounty accordingly.
...
### Workflow
The Open Mined system starts with a Buyer creating a request to Sonar in order to begin a mining campaign.  The Buyer defines some basic parameters:

**Bounty**  
The amount of money the Buyer is willing to spend in order the train their model
...
**Starting Accuracy**  
A claim to the initial accuracy of the default model evaluated on a validation dataset owned by Buyer.

**Target Accuracy**  
*(Default - 0% error)*  
The desired accuracy a dataset should attain once it has been trained by mines, evaluated on the same validation dataset owned by Buyer.

**Algorithm (optional)**  
Which of the algorithms in Syft the Buyer would like to use, optionally including an initial
weight position (in case Buyer wants to start the campaign with a model from a different
campaign).
